
def FanSwitchON(event_struct):
    return


def FanSwitchOFF(event_struct):
    return


def PwControlON(event_struct):
    return


def PwControlOFF(event_struct):
    return


def USBCPowerON(event_struct):
    return


def USBCPowerOFF(event_struct):
    return


def USBAPowerON(event_struct):
    return


def USBAPowerOFF(event_struct):
    return


def USBAAdjust(event_struct):
    return


def USBC3Adjust(event_struct):
    return


def USBC2Adjust(event_struct):
    return


def USBC1Adjust(event_struct):
    return


def WIFIResrt(event_struct):
    return


def WIFIStart(event_struct):
    return


def TempControlON(event_struct):
    return


def TempControlOFF(event_struct):
    return


def TempAdjust(event_struct):
    return


def USBA1Adjust(event_struct):
    return


def USBA2Adjust(event_struct):
    return

